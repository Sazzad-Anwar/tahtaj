    ...
    fetch(`http://tahtaj.r-y-x.net/api/v1/chating/GetChatList?memberId=${memberId}`)
        ...
        .then(data => {
            ...
            if (messages && messages.length) {
                ...
                if (receiverId) {
                    document.querySelector('.receiverUser').innerText = messages.find(message => message.sender.memberId === (receiverId)).sender.firstName + ' ' + messages.find(message => message.sender.memberId === (receiverId)).sender.lastName ?? 'User';
                }
                ...
                messages.forEach(message => {
                    ...
                    document.querySelector('.chatList').insertAdjacentHTML("beforeend", `
                        <div onclick="getChatList(${queryData.memberId}, ${message.sender.memberId}, ${message.postId})"
                            class="chatListCard border-l-4 ${message.sender.memberId === (receiverId) ? 'bg-[#F5F5F5] border-[#00ACE1]' : ''} cursor-pointer normal-transition border-white hover:border-[#00ACE1] hover:bg-[#F5F5F5] px-5 py-4 flex lg:flex-row flex-col items-start lg:items-center justify-start lg:justify-between">
                            <div class="flex">
